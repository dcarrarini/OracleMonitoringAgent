...
public class MySQLInsert {
	...
	private static final Logger log = LoggerFactory.getLogger(MySQLInsert.class);
	...
	public void insertTablespace(Connection MysqlConnection, String sqlInsert, int mode, String table) {
		if (mode == 2) {
			...
			try (PreparedStatement preparedStmt = MysqlConnection.prepareStatement(trucate)) {
				preparedStmt.execute();
			} catch (SQLException e) {
				log.error("MySQLInsert", e);
			}
		}
		try (PreparedStatement preparedStmt = MysqlConnection.prepareStatement(sqlInsert)) {
			preparedStmt.execute();
		} catch (SQLException e) {
			log.error(sqlInsert);
			log.error("MySQLInsert", e);
